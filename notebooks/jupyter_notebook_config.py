...
def output_post_save(model, os_path, contents_manager):
    ...
    dest_dir = pjoin(_BLOG_ROOT, "_posts")
    if not os.path.isdir(dest_dir):
        os.mkdir(dest_dir)
    dest_name = model.get('path').split(".")[0] + ".md"
    dest_path = model.get('path').split(".")[0] + "_files"
    if ftype == "notebook":
        # call nbconvert
        command_str = "jupyter-nbconvert --to markdown {} && mv {}.md {} && (rm -rf {} || true) && (mv {}_files {} ||true)".format(
            os_path, title, pjoin(dest_dir, dest_name),
            pjoin(dest_dir, dest_path),
            title, pjoin(dest_dir, dest_path))
